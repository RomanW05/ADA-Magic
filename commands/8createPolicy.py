...
timeLock = int(sys.argv[3])
isPolicyWithRoyalties = sys.argv[4]

if timeLock == 0:
    timeLock = 315360000
else:
    timeLock = timeLock * 60*60*24
...
#Check if minted token already exists
#if there are no royalties then there is no need in creating a noNameToken
if isPolicyWithRoyalties == 0: sys.exit()
