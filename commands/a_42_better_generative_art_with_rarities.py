...
def store(obj):
    ...
    for x in obj:
        ...
        if ((filename.endswith('png', -3)) or (filename.endswith('PNG', -3))):
            ...
            # array = np.array(x)
            # imageMetadataPair.append(array)
            imageMetadataPair.append(x)
...
def downsize_image(image_open, img_path):
    width, height = image_open.size  # resize to 400*400
    if width > height:
    ...
    x = int(width * ratio)
    y = int(height * ratio)
    image_open = image_open.resize((x,y),Image.ANTIALIAS)
    image_open.save(f'{img_path[:-4]}_thumbnail.png', optimize=True, quality=70)
...
def read_files(current_array, project_name):
    images_and_metadata = []
    for x, image_number in enumerate(current_array):
        x = x + 1
        files = os.listdir(f'/home/yop/Downloads/cardano/vending_machine/{project_name}/layer{x}/')
        files.sort()
        image = files[image_number * 2]
        text_file = files[image_number * 2 + 1]
        image_file = Image.open(f'/home/yop/Downloads/cardano/vending_machine/{project_name}/layer{x}/{image}')
        with open(f'/home/yop/Downloads/cardano/vending_machine/{project_name}/layer{x}/{text_file}', 'r') as text:
            text_metadata = text.read()
        images_and_metadata.append([image_file, text_metadata])
...
def main(project_name):
    ...
    with open(f'/home/yop/Downloads/cardano/vending_machine/{project_name}/current_iteration.txt', 'r') as text:
        current_iteration = text.read()
        current_iteration = current_iteration.replace('\n', '')

    current_array = nfts_array[current_iteration]
    ...
    images_and_metadata = read_files(current_array, project_name)

    for layerDepth, itemNumber in enumerate(current_array):
        newPicture.append(images_and_metadata[layerDepth][0])
        attributes.append(addAttr(images_and_metadata[layerDepth][1]))
    ...
    # Create a thumbnail
    downsize_image(newImage, img_path)
    ...
    with open(f'/home/yop/Downloads/cardano/vending_machine/{project_name}/current_iteration.txt', 'w') as text:
        text.write(str(int(current_iteration)+1))
